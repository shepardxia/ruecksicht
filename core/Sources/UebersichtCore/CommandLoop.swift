...
public actor CommandLoop {
    ...
    public func currentResults() async -> [(id: String, result: TickResult)] {
        await state.latestAll().map { ($0.key, $0.value) }
    }
    ...
    public func refresh(now: TimeInterval) async {
        var live = Set<String>()
        for widget in WidgetDirectory.scan(widgetDirectory) {
                ...
                || schedules[widget.id]?.interval != schedule.interval
            {
                ...
                await scheduler.remove(id: widget.id)
                await scheduler.add(id: widget.id, interval: schedule.interval, now: now)
            }
        }
        for gone in Set(schedules.keys).subtracting(live) {
            schedules.removeValue(forKey: gone)
            await scheduler.remove(id: gone)
            await state.forget(widget: gone)
        }
    }
    ...
    public func tick(now: TimeInterval) async -> [(id: String, result: TickResult)] {
        ...
        for id in await scheduler.due(now: now) {
            ...
            if await state.record(widget: id, result: result) {
